...
def get_weather():
    headers = {"User-Agent": "Delaware All Saints Gospel Radio automation"}
...
# ⭐ FIXED AUDIO — gTTS SAFE CHUNK MODE
def create_audio(script):
    # Split into short safe chunks
    parts = script.split(". ")
    chunk_files = []
    ...
    for i, part in enumerate(parts):
        if part.strip():
            ...
            tts = gTTS(text=part.strip(), lang="en")
            tts.save(filename)
            chunk_files.append(filename)

    # Merge chunks
    output_filename = (
        f"delaware_all_saints_{datetime.datetime.now().strftime('%Y_%m_%d_%H_%M')}.mp3"
    )

    concat = "|".join(chunk_files)
    os.system(
        f"ffmpeg -y -i \"concat:{concat}\" -acodec libmp3lame -ab 128k {output_filename}"
    )
